<template>
  <div>
    <el-card>
      <template #header>
        <div class="card-header">
          <span>消息模板</span>
          <el-button type="primary" @click="dialogVisible = true">添加消息模板</el-button>
        </div>
      </template>
    ...
    <!-- 弹窗 -->
    <el-dialog title="消息模板配置" v-model="dialogVisible" width="500px">
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await axios.get('/api/admin/extend/message_template/list')
