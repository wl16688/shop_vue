<template>
  <div>
    <el-card>
      <template #header>
        <div class="card-header">
          <span>用户等级</span>
          <el-button type="primary" @click="dialogVisible = true">添加用户等级</el-button>
        </div>
      </template>
    ...
    <!-- 弹窗 -->
    <el-dialog title="用户等级配置" v-model="dialogVisible" width="500px">
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await axios.get('/api/admin/extend/user_level/list')
