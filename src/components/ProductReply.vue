<template>
  <div>
    <el-card>
      <template #header>
        <div class="card-header">
          <span>商品评论</span>
          <el-button type="primary" @click="dialogVisible = true">添加商品评论</el-button>
        </div>
      </template>
    ...
    <!-- 弹窗 -->
    <el-dialog title="商品评论配置" v-model="dialogVisible" width="500px">
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await axios.get('/api/admin/extend/product_reply/list')
