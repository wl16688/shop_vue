<template>
  <div>
    <el-card>
      <template #header>
        <div class="card-header">
          <span>管理员列表</span>
          <el-button type="primary" @click="dialogVisible = true">添加管理员列表</el-button>
        </div>
      </template>
    ...
    <!-- 弹窗 -->
    <el-dialog title="管理员列表配置" v-model="dialogVisible" width="500px">
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await axios.get('/api/admin/extend/admin_list/list')
