<template>
  <div>
    <el-card>
      <template #header>
        <div class="card-header">
          <span>发货管理</span>
          <el-button type="primary" @click="dialogVisible = true">添加发货管理</el-button>
        </div>
      </template>
    ...
    <!-- 弹窗 -->
    <el-dialog title="发货管理配置" v-model="dialogVisible" width="500px">
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await axios.get('/api/admin/extend/shipping_list/list')
