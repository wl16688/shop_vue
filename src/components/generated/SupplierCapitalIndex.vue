    ...
    <el-card>
      <template #header>
        <div class="card-header">
          <span>资金流水</span>
          <el-button type="primary" @click="handleAdd">添加资金流水</el-button>
        </div>
      </template>
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await axios.get('/api/admin/extend/supplier_capital_index/list', { params: searchQuery })
...
const handleDetail = async (row) => {
  try {
    const res = await axios.get(`/api/admin/extend/supplier_capital_index/detail?id=${row.id}`)
    if (res.data.code === 200) {
...
const handleStatusChange = async (row) => {
  try {
    const res = await axios.post(`/api/admin/extend/supplier_capital_index/save`, row)
    if (res.data.code === 200) {
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确认删除该记录吗?', '提示', {
    ...
    try {
      const res = await axios.post(`/api/admin/extend/supplier_capital_index/delete`, { id: row.id })
      if (res.data.code === 200) {
...
const handleBatchDelete = () => {
  ...
  ElMessageBox.confirm(`确认删除选中的 ${selectedRows.value.length} 条记录吗?`, '提示', {
    ...
    try {
      const res = await axios.post(`/api/admin/extend/supplier_capital_index/delete`, { ids })
      if (res.data.code === 200) {
...
const handleExport = async () => {
  ...
  try {
    const res = await axios.get(`/api/admin/extend/supplier_capital_index/export`, { responseType: 'blob' })
...
const submitForm = async () => {
  try {
    const res = await axios.post(`/api/admin/extend/supplier_capital_index/save`, form)
    if (res.data.code === 200) {
