<template>
  <div>
    <el-card>
      <template #header>
        <div class="card-header">
          <span>用户设置</span>
          <el-button type="primary" @click="handleAdd">添加用户设置</el-button>
        </div>
      </template>
    ...
    <el-dialog title="用户设置详情" v-model="detailVisible" width="500px">
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await axios.get('/api/admin/extend/user_userSet/list', { params: searchQuery })
...
const handleDetail = async (row) => {
  try {
    const res = await axios.get(`/api/admin/extend/user_userSet/detail?id=${row.id}`)
    if (res.data.code === 200) {
...
const handleStatusChange = async (row) => {
  try {
    const res = await axios.post(`/api/admin/extend/user_userSet/save`, row)
    if (res.data.code === 200) {
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确认删除该记录吗?', '提示', {
    ...
    try {
      const res = await axios.post(`/api/admin/extend/user_userSet/delete`, { id: row.id })
      if (res.data.code === 200) {
...
const handleBatchDelete = () => {
  ...
  ElMessageBox.confirm(`确认删除选中的 ${selectedRows.value.length} 条记录吗?`, '提示', {
    ...
    try {
      const res = await axios.post(`/api/admin/extend/user_userSet/delete`, { ids })
      if (res.data.code === 200) {
...
const handleExport = async () => {
  ...
  try {
    const res = await axios.get(`/api/admin/extend/user_userSet/export`, { responseType: 'blob' })
    const url = window.URL.createObjectURL(new Blob([res.data]))
    const link = document.createElement('a')
    link.href = url
    link.setAttribute('download', '用户设置_export.csv')
...
const submitForm = async () => {
  try {
    const res = await axios.post(`/api/admin/extend/user_userSet/save`, form)
    if (res.data.code === 200) {
