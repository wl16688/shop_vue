...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await axios.get('/api/admin/extend/setting_document/list', { params: searchQuery })
...
const handleDetail = async (row) => {
  try {
    const res = await axios.get(`/api/admin/extend/setting_document/detail?id=${row.id}`)
    if (res.data.code === 200) {
...
const handleStatusChange = async (row) => {
  try {
    const res = await axios.post(`/api/admin/extend/setting_document/save`, row)
    if (res.data.code === 200) {
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确认删除该记录吗?', '提示', {
    ...
    try {
      const res = await axios.post(`/api/admin/extend/setting_document/delete`, { id: row.id })
      if (res.data.code === 200) {
...
const handleBatchDelete = () => {
  ...
  ElMessageBox.confirm(`确认删除选中的 ${selectedRows.value.length} 条记录吗?`, '提示', {
    ...
    try {
      const res = await axios.post(`/api/admin/extend/setting_document/delete`, { ids })
      if (res.data.code === 200) {
...
const handleExport = async () => {
  ...
  try {
    const res = await axios.get(`/api/admin/extend/setting_document/export`, { responseType: 'blob' })
...
const submitForm = async () => {
  try {
    const res = await axios.post(`/api/admin/extend/setting_document/save`, form)
    if (res.data.code === 200) {
