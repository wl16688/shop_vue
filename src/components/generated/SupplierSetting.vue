<template>
  <div>
    <el-card>
      <template #header>
        <div class="card-header">
          <span>供应商设置</span>
          <el-button type="primary" @click="handleAdd">添加供应商设置</el-button>
        </div>
      </template>
    ...
    <el-dialog title="供应商设置详情" v-model="detailVisible" width="500px">
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await axios.get('/api/admin/extend/supplier_setting/list', { params: searchQuery })
...
const handleDetail = async (row) => {
  try {
    const res = await axios.get(`/api/admin/extend/supplier_setting/detail?id=${row.id}`)
    if (res.data.code === 200) {
...
const handleStatusChange = async (row) => {
  try {
    const res = await axios.post(`/api/admin/extend/supplier_setting/save`, row)
    if (res.data.code === 200) {
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确认删除该记录吗?', '提示', {
    ...
    try {
      const res = await axios.post(`/api/admin/extend/supplier_setting/delete`, { id: row.id })
      if (res.data.code === 200) {
...
const handleBatchDelete = () => {
  ...
  ElMessageBox.confirm(`确认删除选中的 ${selectedRows.value.length} 条记录吗?`, '提示', {
    ...
    try {
      const res = await axios.post(`/api/admin/extend/supplier_setting/delete`, { ids })
      if (res.data.code === 200) {
...
const handleExport = async () => {
  ...
  try {
    const res = await axios.get(`/api/admin/extend/supplier_setting/export`, { responseType: 'blob' })
    const url = window.URL.createObjectURL(new Blob([res.data]))
    const link = document.createElement('a')
    link.href = url
    link.setAttribute('download', '供应商设置_export.csv')
...
const submitForm = async () => {
  try {
    const res = await axios.post(`/api/admin/extend/supplier_setting/save`, form)
    if (res.data.code === 200) {
