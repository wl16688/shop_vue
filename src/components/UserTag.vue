<template>
  <div>
    <el-card>
      <template #header>
        <div class="card-header">
          <span>用户标签</span>
          <el-button type="primary" @click="dialogVisible = true">添加用户标签</el-button>
        </div>
      </template>
    ...
    <!-- 弹窗 -->
    <el-dialog title="用户标签配置" v-model="dialogVisible" width="500px">
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await axios.get('/api/admin/extend/user_tag/list')
